...
function list(req, res, next) {
    Catalog.find().then(objs => res.status(200).json({
        ...
        message:"No se puedo consultar la lista de catalogs",
        obj:ex
    }));
}
...
function update(req, res, next){
    ...
    Catalog.findOneAndUpdate({"_id":id}, catalog)
            .then(obj => res.status(200).json({
                message:`Catalogo actualizada corretamente, con el id: ${id}`,
                obj:obj
            })).catch(ex => res.status(500).json({
...
function destroy(req, res, next){
    ...
    Catalog.findByIdAndRemove({"_id":id}).then(obj => res.status(200).json({
        ...
        message:`No se puedo eliminar el catalog con el id: ${id}`,
        obj:ex
    }));
}
